...
class Bot:
    ...
    def __crawler_olx(self, query, pages=0):
        ...
        if name_n_link:
            ...
            return 'Nenhum anúncio encontrado com as palavras-chaves'
        try:
            ...
            for j in i:
                try:
                    if valor_maximo > 0:
                        ...
                        if valor_minimo < int(j[2][3:].translate({ord('.'): None})) and set(filtro) <= set(j[0].lower().split()) if len(filtro) > 0 else True:
                            tabela.append(j)
                except ValueError:
                    ...
...
bot.get_ads_olx('iphone', valor_maximo=2000, filtro=filtro_)